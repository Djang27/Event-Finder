const axios = require("axios");

const TM_API_KEY = process.env.TICKETMASTER_API_KEY;
const BASE_URL = "https://app.ticketmaster.com/discovery/v2";

async function fetchEvents({ city, startDate, endDate, category, keyword }) {
  const params = {
    apikey: TM_API_KEY,
    city,
    ...(startDate && { startDateTime: startDate }),
    ...(endDate && { endDateTime: endDate }),
    classificationName: category,
    keyword,
    size: 20,
    includeSpellcheck: "yes",
    expand: "priceRanges",
  };

  const response = await axios.get(`${BASE_URL}/events.json`, { params });
  const events = response.data._embedded?.events || [];

  return events
    .filter((e) => e.url && (
      e.url.startsWith("https://www.ticketmaster.com") ||
      e.url.startsWith("https://www.axs.com")
    ))
    .map((e) => ({
      id: `tm-${e.id}`,
      source: "ticketmaster",
      name: e.name,
      date: e.dates?.start?.localDate,
      time: e.dates?.start?.localTime,
      venue: e._embedded?.venues?.[0]?.name,
      city: e._embedded?.venues?.[0]?.city?.name,
      lat: e._embedded?.venues?.[0]?.location?.latitude,
      lng: e._embedded?.venues?.[0]?.location?.longitude,
      price: e.priceRanges?.find(p =>
        p.type === "standard" || p.type === "including fees"
      )?.min || e.priceRanges?.[0]?.min || null,
      url: e.url,
      image: e.images?.[0]?.url,
      promoter: e.promoter?.name || null,
    }));
}

module.exports = { fetchEvents };